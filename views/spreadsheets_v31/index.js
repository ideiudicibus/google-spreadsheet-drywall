...
exports.readPopulateActiveSheet = function(req, res, next){
  ...
  req.app.db.models.Spreadsheet.findOne({_id:req.params.id}).populate('sheetsList').exec(function(err, spreadsheet) {
    ...
    if (req.xhr) {
      ...
      res.render('spreadsheets_v31/dashboard/index-dashboard', { data: { record: spreadsheet,title:spreadsheet.name} });
    }
  });
};
...
exports.getPrintablePage = function(req, res, next){
  ...
  req.app.db.models.Spreadsheet.findOne({_id:req.params.id}).populate('sheetsList').exec(function(err, spreadsheet) {
    if (err) {
      ...
      //console.log('getPrintablePage invoked spreadsheets_v31');
      return res.render('spreadsheets_v31/dashboard/index-printable', { data: { record: spreadsheet,title:spreadsheet.name} });
  });
};
