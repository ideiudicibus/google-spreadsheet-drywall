...
elem.toggleClass('active');
     ...
     $.ajax({        
        type: "POST",
        data: data,
        url: '/spreadsheets_v31/activesheet/g/'+spreadsheet.activeSheet,
        dataType:"json",
        statusCode:
            {
        ...
        success:  function (response) {
          ...
          { 
           ...
           templateLoader.loadRemoteTemplate(activeSheet._id, "/views/spreadsheets_v31/dashboard/"+activeSheet._id+"-tmpl.html?", 
            function(data) {
...
if($(this).attr('class').indexOf('checkPercentageValue')>0){
 ...
 $.ajax({
        ...
        data:params,
        type: "POST",
        url: '/spreadsheets_v31/'+data.record._id+'/'+data.record.activeSheet+'/params',
        dataType:"json",
        success:  function (response) {
...
function loadUpdateParameterBtnBehaviourFullP04() {
...
function loadUpdateParameterBtnBehaviourFullP03() {
...
function loadUpdateParameterBtnBehaviourFullP01() {
...
params.activeSheet=data.record.activeSheet;
 ...
 $.ajax({
        ...
        data:params,
        type: "POST",
        url: '/spreadsheets_v31/'+data.record._id+'/'+data.record.activeSheet+'/reset',
        dataType:"json",
        success:  function (response) {
  ...
  event.preventDefault();
    ...
    if (e) {
      if (str.length==0 || str.length>50){alertify.error('la descrizione della simulazione è vuota oppure eccede i 50 caratteri previsti, riprovare'); return false; }
       ...
       data:postData,
        type: "POST",
        url: '/spreadsheets_v31/'+data.record._id+'/s/simulation',
        dataType:"json",
        success:  function (response) {
...
function resetParamsActiveSheetBtnBehaviour(){
  ...
  event.preventDefault();
    ...
    if (e) {
       ...
       data:data,
        type: "POST",
        url: '/spreadsheets_v31/activesheet/r/'+data.record.activeSheet,
        dataType:"json",
        success:  function (response) {
...
function loadSimulation(simId){
  ...
  $.ajax({
        data:data,
        type: "POST",
        url: '/spreadsheets_v31/'+data.record._id+'/l/simulation/'+simId,
        success:  function (response) {
...
$.ajax({
        ...
        url: '/spreadsheets_v31/'+data.record._id+'/g/simulations',
        success:  function (response) {
